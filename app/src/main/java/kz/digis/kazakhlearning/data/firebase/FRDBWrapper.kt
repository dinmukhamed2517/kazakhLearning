...
abstract class FRDBWrapper<T> {
    ...
    fun saveLastName(value: String) {
        db.getReference(getTableName()).child("lastname").setValue(value)
    }

    fun saveAppTime(value:Int){
        db.getReference(getTableName()).child("dailyTime").setValue(value)
    }

    fun saveAchievement(value: Achievement){
